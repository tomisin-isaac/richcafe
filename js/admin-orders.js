document.addEventListener("DOMContentLoaded", () => {
	...
	const closeFilterModalBtn = filterOrdersModal
		? filterOrdersModal.querySelector(".close-button")
		: null;
	...
	function renderOrders(filteredOrders = orders) {
		if (!ordersTableBody) {
			console.error(
				"admin-orders.js: ordersTableBody not found. Cannot render orders."
			);
			return;
		}
		...
		if (filteredOrders.length === 0) {
			ordersTableBody.innerHTML =
				'<tr><td colspan="10" style="text-align: center; padding: 2rem;">No orders found.</td></tr>';
			return;
		}

		filteredOrders.forEach((order, index) => {
			// Use index for 'No' column
			...
			row.innerHTML = `
                ...
                <td>
                    <i class="fas fa-eye view-order-icon" data-id="${
											order.id
	...
	if (ordersSearchInput) {
		ordersSearchInput.addEventListener("keyup", (event) => {
			...
			console.log(
				`admin-orders.js: Orders searched for: "${searchTerm}". ${filtered.length} results.`
			);
		});
	}
	...
	if (filterOrdersForm) {
		filterOrdersForm.addEventListener("submit", (event) => {
			...
			if (startDate && endDate) {
				...
				console.log(
					`admin-orders.js: Filtered by date range: ${startDate} to ${endDate}. ${filtered.length} results.`
				);
			} else if (startDate) {
				const start = new Date(startDate);
				filtered = orders.filter((order) => {
				...
				console.log(
					`admin-orders.js: Filtered by start date: ${startDate}. ${filtered.length} results.`
				);
			} else if (endDate) {
				const end = new Date(endDate);
				end.setDate(end.getDate() + 1); // Include the end date fully
				...
				console.log(
					`admin-orders.js: Filtered by end date: ${endDate}. ${filtered.length} results.`
				);
			} else {
				console.log(
					"admin-orders.js: No dates selected for filter. Showing all orders."
				);
			}
	...
	try {
		...
		alert(
			"An error occurred during orders page initialization. Please check the console for details."
		);
	}
		...
		ordersTableBody.addEventListener("click", (event) => {
			if (event.target.classList.contains("view-order-icon")) {
				...
				if (orderDetails) {
					...
					console.log(
						`admin-orders.js: View icon clicked for Order ID: ${orderId}`
					);
				}
			}
		});
