...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const itemId = qs.get('id') || sessionStorage.getItem('selectedProductId') || null;
  ...
  function computeModalTotal() {
    return modalCart.reduce((sum, it) => sum + (Number(it.price || 0) * Number(it.quantity || 0)), 0);
  }
  ...
  const catName = getCategoryName(currentItem.categoryId, currentItem.categoryName);
  if (categoryTitleEl) categoryTitleEl.textContent = catName;
  if (prepTimeEl) prepTimeEl.textContent = currentItem.prepTime ? `${currentItem.prepTime} mins` : '—';
  ...
  function renderProductRow(item, isSelected = false) {
    ...
    const imgBg = item.imageUrl ? `style="background-image:url('${item.imageUrl}');"` : '';
      ...
      <div class="product-item-left">
        <div>
          ...
          <div class="product-item-subtitle">
            ...
            ${hasStockNote ? `<span class="product-stock">${stockTxt}</span>` : ''}
          </div>
        </div>
        ${sizesHtml}
      </div>
  ...
  const related = menuItems.filter(i => i.categoryId === currentItem.categoryId && i.id !== currentItem.id);
  ...
  function findModalCartIndex(itemId, sizeLabel) {
    return modalCart.findIndex(it => it.id === itemId && it.size === sizeLabel);
  }
    ...
    modalCart.forEach((it, idx) => {
      ...
      row.innerHTML = `
        <div class="cart-item-left">
          <img src="${it.imageUrl || 'https://via.placeholder.com/50'}" alt="${it.name}" style="width:44px;height:44px;border-radius:6px;object-fit:cover;"/>
          <div>
            <div class="cart-item-name">${it.name}</div>
            <div class="cart-item-size">${it.size} • ${formatNaira(it.price)}</div>
          </div>
        </div>
        <div class="cart-item-right">
          ...
          <div style="min-width:7rem; text-align:right; font-weight:700; margin-left:.6rem;">${formatNaira(it.price * it.quantity)}</div>
  ...
  function renderModalCategories() {
    ...
    const visibleCategories = categories.length ? categories : Object.keys(byCategory).map(id => ({ id, name: id }));
        ...
        itemRow.innerHTML = `
          <div class="item-left">
            ...
            <div>
              ...
              <div style="font-size:.85rem;color:#666;">${item.sizes && item.sizes.length ? item.sizes.map(s=>s.size+': '+formatNaira(s.price)).join(' • ') : ''}</div>
            </div>
          </div>
          <div class="item-right" style="display:flex; align-items:center; gap:.5rem;">
            <button class="modal-item-add-btn" data-id="${item.id}">+ Add</button>
          </div>
        `;
    ...
    modalCategories.querySelectorAll('.modal-category-header').forEach(h => {
      h.addEventListener('click', () => {
        ...
        const container = modalCategories.querySelector(`.modal-category-items[data-cat-items="${catId}"]`);
        if (!container) return;
    ...
    modalCategories.querySelectorAll('.modal-item-add-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const sizeObj = (itemObj.sizes && itemObj.sizes[0]) ? itemObj.sizes[0] : { size: 'Default', price: 0 };
  ...
  try {
    ...
    if (editPayload) {
      try {
        ...
        modalCart = Array.isArray(JSON.parse(editPayload)) ? JSON.parse(editPayload) : [];
        // remove saved payload so it doesn't persist
        sessionStorage.removeItem('cartEditItem');
      } catch (e) {
        console.warn('product-details.js: failed to parse cartEditItem payload', e);
      }
    }
    if (shouldOpen === 'true') {
