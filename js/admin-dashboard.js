document.addEventListener('DOMContentLoaded', function() {
    ...
    if (salesCtx) {
        const salesTimeframeSelect = document.querySelector('.sales-figures-card .timeframe-select');
        ...
        function getSalesData(timeframe) {
            ...
            if (timeframe === 'daily') {
                ...
                return {
                    ...
                    actualData: [300000, 210000, 280000, 440000, 120000, 600000, 380000, 210000, 50000, 350000, 510000, 620000],
                    maxData: monthlyMax,
        ...
        function updateSalesChart() {
            ...
            const { labels, actualData, maxData, labelFormat } = getSalesData(selectedTimeframe);
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    if (context.datasetIndex === 0) { // Only show label for the actual sales data
    ...
    if (ordersCtx) {
        const orderTimeframeSelect = document.getElementById('orderTimeframeSelect');
        ...
        function updateOrdersDonutChart() {
            ...
            const { total, breakdown, labels, labelText } = getOrdersData(selectedTimeframe);
            ...
            const donutSegmentLightPink = style.getPropertyValue('--donut-segment-light-pink').trim(); // Burger
            const donutSegmentDarkBlue = style.getPropertyValue('--donut-segment-dark-blue').trim(); // Milkshake
            const cardBackground = style.getPropertyValue('--card-background-light').trim(); // Border color for donut
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMenuItemCount() {
        ...
        if (totalMenuItemsElement) {
            ...
            console.log(`admin-dashboard.js: Updated menu item count to ${menuItems.length}`);
        } else {
            console.log("admin-dashboard.js: Element with ID 'menuItemCount' not found on this page.");
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const totalCustomersCountElement = document.getElementById('totalCustomersCount');
    ...
    if (totalCustomersCountElement) {
        ...
        console.error("Element with ID 'totalCustomersCount' not found in admin-overview.html.");
    }
});