document.addEventListener('DOMContentLoaded', () => {
    ...
    const operationalStatusIndicator = document.getElementById('operationalStatusIndicator');
    const operationalStatusText = document.getElementById('operationalStatusText');
    const toggleOperationalHoursBtn = document.getElementById('toggleOperationalHours');
    ...
    function updateOperationalStatusUI(status) {
        if (!operationalStatusIndicator || !operationalStatusText) {
            console.error("admin-settings.js: Operational status UI elements not found.");
            return;
        }
        if (status === 'open') {
    ...
    if (toggleOperationalHoursBtn) {
        toggleOperationalHoursBtn.addEventListener('click', () => {
            currentOperationalStatus = (currentOperationalStatus === 'open') ? 'closed' : 'open';
            saveOperationalStatus(currentOperationalStatus);
            updateOperationalStatusUI(currentOperationalStatus);
            alert(`Business is now ${currentOperationalStatus === 'open' ? 'Open' : 'Closed'} for Business.`);
        });
    }
    ...
    const closeLocationModalBtn = locationModal ? locationModal.querySelector('.close-button') : null;
    ...
    function loadLocations() {
        ...
        try {
            ...
            console.error("admin-settings.js: Error parsing locations from localStorage:", e);
    ...
    function renderLocations() {
        if (!locationsTableBody) {
            console.error("admin-settings.js: locationsTableBody not found. Cannot render locations.");
            return;
        }
        locationsTableBody.innerHTML = ''; // Clear existing rows

        if (locations.length === 0) {
            locationsTableBody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem;">No delivery locations added yet.</td></tr>';
            return;
        }
        ...
        locations.forEach((location, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <i class="fas fa-edit edit-icon" data-id="${location.id}" title="Edit Location"></i>
                    <i class="fas fa-trash-alt delete-icon" data-id="${location.id}" title="Delete Location"></i>
                </td>
            `;
    ...
    if (locationsTableBody) {
        locationsTableBody.addEventListener('click', (event) => {
            ...
            // Delete icon click
                ...
                if (locationToDelete && confirm(`Are you sure you want to delete location "${locationToDelete.name}"?`)) {
